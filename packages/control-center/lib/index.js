...
function htmlToText(html) {
	return html.replace(/<script[\s\S]*?<\/script>/gi, " ").replace(/<style[\s\S]*?<\/style>/gi, " ").replace(/<!--[\s\S]*?-->/g, " ").replace(/<\/(p|div|section|article|h[1-6]|li|tr|br)>/gi, "\n").replace(/<br\s*\/?>/gi, "\n").replace(/<[^>]+>/g, "").replace(/&nbsp;/g, " ").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, "\"").replace(/&#39;/g, "'").replace(/\n{3,}/g, "\n\n").trim();
}
...
function createBrowser() {
	...
	server.registerTool("fetch_page", {
		...
		inputSchema: z.object({
			...
			maxChars: z.number().int().min(500).max(5e4).optional().describe("返回文本的最大字符数（默认 8000）")
		})
	}, async (args) => {
		let url;
		try {
		...
		try {
			...
			if (!response.ok) return { content: [{
				type: "text",
				text: `抓取失败：HTTP ${String(response.status)}`
			}] };
			...
			if (buffer.byteLength > MAX_BYTES) return {
				...
				content: [{
					type: "text",
					text: `页面过大（${String(Math.round(buffer.byteLength / 1024))}KB），上限 2048KB`
				}]
			};
			const body = new TextDecoder().decode(buffer);
			const title = /<title[^>]*>([\s\S]*?)<\/title>/i.exec(body)?.[1]?.trim() ?? "";
			const fullText = contentType.includes("html") || /^\s*<(!doctype|html)/i.test(body) ? htmlToText(body) : body.trim();
			if (fullText.length === 0) return { content: [{
				type: "text",
				text: "页面无可提取文本"
			}] };
			const maxChars = typeof args.maxChars === "number" ? args.maxChars : 8e3;
			const clipped = fullText.length > maxChars ? `${fullText.slice(0, maxChars)}\n\n[已截断，全文 ${String(fullText.length)} 字符]` : fullText;
			return { content: [{
				type: "text",
			...
			return {
				...
				content: [{
					type: "text",
					text: `抓取失败：${error instanceof Error ? error.message : String(error)}`
				}]
			};
		}
	});
...
const updateBundleDomain = defineDomain({
	...
	version: 1,
	tables: { bundles: domainTable(bundleSchema) }
});
	...
	async listReleases() {
		try {
			...
			try {
				...
				return {
					ok: true,
					value: (await response.json()).map((release) => ({
...
//#endregion
//#region lib/types/notes-remote-client.js
...
const GENERAL_SCHEMA = Schema.object({
	...
	proxyMode: Schema.string().default("off"),
