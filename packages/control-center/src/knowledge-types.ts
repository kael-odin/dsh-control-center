...
declare module '@deepseek-ai/dsh-typert-protocol' {
  interface TypertRemoteNamespaceMap {
    controlCenterKnowledge: {
      listBases(): Promise<{ ok: true; value: { bases: KnowledgeBaseView[] } } | { ok: false; error: { code: string; message: string; details: object } }>
      createBase(request: KnowledgeCreateBaseRequest): Promise<{ ok: true; value: KnowledgeBaseView } | { ok: false; error: { code: string; message: string; details: object } }>
      getBase(baseId: KnowledgeBaseId): Promise<{ ok: true; value: KnowledgeBaseView } | { ok: false; error: { code: string; message: string; details: object } }>
      deleteBase(baseId: KnowledgeBaseId): Promise<{ ok: true; value: { absent: true } } | { ok: false; error: { code: string; message: string; details: object } }>
      ...
      listChunks(baseId: KnowledgeBaseId, cursor: string | null, limit: number): Promise<{ ok: true; value: { chunks: KnowledgeChunkView[]; nextCursor?: string } } | { ok: false; error: { code: string; message: string; details: object } }>
      retrieve(request: KnowledgeRetrieveRequest): Promise<{ ok: true; value: KnowledgeRetrievalResult } | { ok: false; error: { code: string; message: string; details: object } }>
    }
  }
}
