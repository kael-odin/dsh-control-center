...
| 能力 | MCP | ⚠️ | **真实子导航（Cherry McpSettingsPage parity，2026-08-24）**：服务器（分栏列表+详情 tab）/ 内置服务器（预设页，直接添加）/ 市场（npx scope 搜索 + 外部市场站点）。**仍缺**：提供商配置子页（Bailian/ModelScope 托管 MCP 发现）、协议安装向导、Cherry 9 个 inMemory 内置 server（memory/sequential-thinking/fetch/filesystem/brave-search/python/dify-knowledge/browser/didi，需 host 侧真实实现） |
