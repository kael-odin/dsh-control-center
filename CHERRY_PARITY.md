...
|---|------------------|-----------------|---------------------|------------------|------|----------|
| 2.2.1 | 供应商列表 | 左栏全量目录，按分组渲染（国内/国际/本地），点击选中并持久化 `last_selected_provider_id` | ✅ `ProviderDirectorySection` 左栏：静态 61 预设分组 + `llm.providers()` 状态 join，选中持久化 `settings.provider.last_selected_provider_id` | `llm.providers()`（状态 join） | — | 点任一供应商，右栏切换并刷新后保持选中 ✅ |
| 2.2.2 | 搜索 | ProviderListSearchField 过滤列表 | ✅ 左栏搜索框本地过滤（id/name） | 同上 | — | 输入关键字过滤列表 ✅ |
...
| 2.2.4 | 右键菜单 | ProviderListItemWithContextMenu：编辑 / 删除 / 启用 | 🔶 删除有确认流程（`removeProviderProfile`，ModelsSection 已有）；两栏右栏 Header 未挂删除入口 | 已有 | 后续：右栏 Header 加删除 / 右键菜单 | 右键供应商出现编辑/删除菜单 |
| 2.2.5 | 启用开关 | 每个供应商可整体禁用 | 🔶 右栏显示 active/未配置 真实态（来自 `llm.providers()`）；「启用」开关语义不清晰（pi-ai 无独立 enabled 字段）未做 | settings 命名空间 | 后续：定义 enabled 语义再实现 | 切换后目录项真实禁用 |
...
|---|-------------|-----------------|---------------------|------|----------|
| 2.3.1 | ProviderHeader | 名称（可链官网）+ 启用 Switch + 闪电图标打开 API 选项抽屉 | ✅ `ProviderDirectorySection` 右栏 Header：名称 + route + 自定义标签 + active 态；协议以行内 select 呈现（非抽屉）；官网链接未做 | 后续：官网链接 / 抽屉化 | 右栏顶部显示名称/启用态 ✅ |
| 2.3.2 | 认证区 | API key 输入/保存、连接测试、"获取模型"引导动画 | ✅ `ProviderEditor` 凭据表单（走 DSH credentials）+ baseURL/协议预填 +「获取可用模型」真实 `llm.discoverModels` 探测（即连接测试，非目录路由走 wire interrogation）；引导动画未做 | 后续：引导动画 | 填 key → 测试 → 真实 HTTP 探测并反馈 ✅ |
| 2.3.3 | 模型列表 | 该供应商模型列表：启用/禁用、搜索、添加、从供应商拉取、默认标记、健康状态 | ✅ `ModelListEditor`：增删/容量/拉取（发现候选多选采纳）；搜索与默认标记未做 | 后续：搜索/默认标记 | 模型列表真实增删、可拉取 ✅ |
...
control-center 现状：✅ SkillsSection.tsx（卡片网格 + 搜索 + 启用/禁用/卸载；市场安装真，本地导入诚实门控"当前平台不支持：本地导入需要宿主文件系统路径"）。
